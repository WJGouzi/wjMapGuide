...
@synthesize window, background, hud, spinner, image, label, indicateImage;
...
+ (void)showWaitingWithImage:(NSString *)status {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        UIImage *indicateImages = [UIImage animatedImageWithAnimatedGIFURL:url];
        [[self shared] hudCreate:status image:indicateImages spin:NO hide:NO interaction:YES];
    });
}
